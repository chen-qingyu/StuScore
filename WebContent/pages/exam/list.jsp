						...
						<tbody>
							<c:forEach var="data" items="${page.data }" varStatus="index">
								<tr>
									...
									<td>${data.tname }</td>
									<td>
										<button value="${data.id }" gid="${data.gid }" class="btn btn-score" type="button">
						...
						<c:choose>
							<c:when test="${index == page.pageIndex }">
								<li class="page-panel-this">
									<a href="/stuscore/exam?action=page&pageIndex=${index }&pageSize=${page.pageSize }">${index }</a>
								</li>
							</c:when>
							<c:otherwise>
								<li>
	...
	<script type="text/javascript">
		$(function(){
			...
			$(".btn-search").on("click", function(e){
				...
				window.location.href = "/stuscore/exam?action=page&pageIndex=1&pageSize=10&name=" + qryText;
			});
			...
			$(".btn-score").on("click", function(e){
				...
				window.location.href = "/stuscore/exam?action=scoreView&gId=" + gId + "&eId=" + eId;
			});
			
			// 添加按钮绑定
			$(".btn-add").on("click", function(e){
				
				window.location.href = "/stuscore/exam?action=addView";
			});
			...
			$(".btn-edit").on("click", function(e){
				...
				window.location.href = "/stuscore/exam?action=updView&id=" + id;
			});
			...
			$(".btn-del").on("click", function(){
				...
				window.location.href = "/stuscore/exam?action=del&id=" + id;
			});
		});
	</script>
</body>
